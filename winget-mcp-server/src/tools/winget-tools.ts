...
function runWinget(args: string[], timeout = 60_000): Promise<{ stdout: string; stderr: string; code: number }> {
...
export function registerWinGetTools(server: Server): void {
        ...
        try {
            switch (name) {
                case 'winget_search': {
                    ...
                    if (count > 0) argsArr.push('--count', String(count));
                    if (exact) argsArr.push('--exact');
                    if (source) argsArr.push('--source', source);
                ...
                case 'winget_show': {
                    ...
                    if (versions) argsArr.push('--versions');
                    const r = await runWinget(argsArr);
                    const text = r.stdout || r.stderr || 'No output.';
                    return {
                        content: [{ type: 'text', text }],
                ...
                case 'winget_list': {
                    ...
                    if (count != null && count > 0) argsArr.push('--count', String(count));
                    if (query) argsArr.push('--query', query);
                    const r = await runWinget(argsArr);
                    const text = r.stdout || r.stderr || 'No output.';
                ...
                case 'winget_list_upgradable': {
                    ...
                    if (includeUnknown) argsArr.push('--include-unknown');
                    const r = await runWinget(argsArr);
                    const text = r.stdout || r.stderr || 'No output.';
                    return { content: [{ type: 'text', text }] };
                }
