...
import {
    ...
    generateReadme, generateChangelog, generateFullModule,
} from '../services/template-engine.js';
import {
    DEFAULT_AUTHOR, DEFAULT_VERSION, DEFAULT_LICENSE,
    BETTER11_NS, BETTER11_PREFIX,
} from '../constants.js';
import type {
    PsModuleConfig, CsViewModelConfig, CsServiceConfig,
    ReadmeConfig, ChangelogConfig, FullModuleConfig,
    ScaffoldResult, TemplateInfo,
} from '../types.js';

const CHARACTER_LIMIT = 40_000;
const naming = new NamingService();

function formatResult(result: ScaffoldResult): string {
    if (!result.success) {
        return `❌ Scaffolding failed:\n${result.errors.join('\n')}`;
    }

    const fileList = result.files.map(f =>
        `  📄 ${f.relativePath} (${f.language}, ${f.content.split('\n').length} lines)`
    ).join('\n');
...
export function registerScaffolderTools(server: McpServer): void {
...
Returns: Generated file contents with paths.`,
        ...
        async (params) => {
            try {
                const result = generatePsModule(params as PsModuleConfig);
...
Returns: Interface, implementation, and test files.`,
    ...
    server.registerTool(
        ...
        async (params) => {
            try {
                const result = generateReadme(params as ReadmeConfig);
                return { content: [{ type: 'text' as const, text: formatResult(result) }] };
            } catch (err: unknown) {
                const msg = err instanceof Error ? err.message : String(err);
                return { content: [{ type: 'text' as const, text: `❌ Error: ${msg}` }], isError: true };
    ...
    server.registerTool(
        ...
        async (params) => {
            try {
                const result = generateChangelog(params as ChangelogConfig);
                return { content: [{ type: 'text' as const, text: formatResult(result) }] };
            } catch (err: unknown) {
                const msg = err instanceof Error ? err.message : String(err);
                return { content: [{ type: 'text' as const, text: `❌ Error: ${msg}` }], isError: true };
    ...
    server.registerTool(
        ...
        async (params) => {
            ...
            if (result.variants) {
                output.push(
                    '', '  Name variants:',
                    `    PascalCase:  ${result.variants.pascal}`,
                    `    camelCase:   ${result.variants.camel}`,
                    `    kebab-case:  ${result.variants.kebab}`,
                    `    snake_case:  ${result.variants.snake}`,
                    `    Display:     ${result.variants.display}`,
                );
            }
