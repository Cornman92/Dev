...
function renderParam(p) {
    ...
    const pAttrs = [];
    if (p.mandatory)
        pAttrs.push('Mandatory = $true');
    if (p.position >= 0)
        pAttrs.push(`Position = ${p.position}`);
    if (p.helpMessage)
        pAttrs.push(`HelpMessage = '${p.helpMessage.replace(/'/g, "''")}'`);
...
function renderManifest(config, modName) {
...
function renderPester(config, modName) {
...
export function generatePsModule(config) {
    try {
        const files = [];
        ...
        files.push({ relativePath: `${modName}/${modName}.psm1`, language: 'powershell', content: `#Requires -Version 5.1\nSet-StrictMode -Version Latest\n$ErrorActionPreference = 'Stop'\n\n${fnsCode}\n\nExport-ModuleMember -Function @(\n    ${exports}\n)\n` });
        if (config.includeManifest)
            files.push({ relativePath: `${modName}/${modName}.psd1`, language: 'powershell', content: renderManifest(config, modName) });
        if (config.includeTests)
            files.push({ relativePath: `${modName}/Tests/${modName}.Tests.ps1`, language: 'powershell', content: renderPester(config, modName) });
...
export function generateCsViewModel(config) {
    try {
        const files = [];
        ...
        if (config.includeNavigation)
            usings.push(`using ${BETTER11_NS}.Navigation;`);
        if (config.includeValidation)
            usings.push('using System.ComponentModel.DataAnnotations;');
        ...
        files.push({ relativePath: `ViewModels/${cn}.cs`, language: 'csharp', content: `${usings.join('\n')}\n\nnamespace ${ns};\n\n/// <summary>${config.description}</summary>\npublic partial class ${cn} : ${base}\n{\n${fields}\n\n    public ${cn}(${ctorP})\n    {\n${ctorA}\n    }\n\n    #region Properties\n\n${props}\n\n    #endregion\n\n    #region Commands\n\n${cmds}\n\n    #endregion\n}\n` });
...
export function generateCsService(config) {
    try {
        const files = [];
        ...
        if (config.includeLogging)
            cas.push('        _logger = logger ?? throw new ArgumentNullException(nameof(logger));');
        ...
        files.push({ relativePath: `Services/${cn}.cs`, language: 'csharp', content: `${usings.join('\n')}\n\nnamespace ${ns};\n\n/// <summary>${config.description}\n/// Lifetime: ${config.lifetime}</summary>\npublic class ${cn} : ${iName}\n{\n${flds.join('\n')}\n\n    public ${cn}(${cps.join(', ')})\n    {\n${cas.join('\n')}\n    }\n\n${iMthds}\n}\n` });
        // DI snippet
        ...
        if (config.includeTests) {
            ...
            files.push({ relativePath: `Tests/${cn}Tests.cs`, language: 'csharp', content: `using System;\nusing System.Threading.Tasks;\nusing Xunit;\n\nnamespace ${ns}.Tests;\n\npublic class ${cn}Tests\n{\n    private static ${cn} CreateSut() => null!;\n\n${tMethods}\n}\n` });
        }
...
export function generateFullModule(config) {
    const allFiles = [];
    const allErrors = [];
    const summaries = [];
    ...
    if (psResult.success) {
        allFiles.push(...psResult.files);
        summaries.push(psResult.summary);
    }
    else
        allErrors.push(...psResult.errors);
    if (config.csViewModel.className) {
        ...
        if (vmResult.success) {
            allFiles.push(...vmResult.files);
            summaries.push(vmResult.summary);
        }
        else
            allErrors.push(...vmResult.errors);
    }
    if (config.csService.className) {
        ...
        if (svcResult.success) {
            allFiles.push(...svcResult.files);
            summaries.push(svcResult.summary);
        }
        else
            allErrors.push(...svcResult.errors);
    }
    if (config.includeReadme) {
        ...
        if (rr.success) {
            allFiles.push(...rr.files);
            summaries.push(rr.summary);
        }
    }
