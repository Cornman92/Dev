...
export function generateNameVariants(input) {
    const words = splitName(input);
    if (words.length === 0)
        return { pascal: '', camel: '', kebab: '', snake: '', display: '' };
    return {
...
export function toPsVerbNoun(verb, noun) {
    return `${generateNameVariants(verb).pascal}-${generateNameVariants(noun).pascal}`;
}
export function toInterfaceName(className) {
