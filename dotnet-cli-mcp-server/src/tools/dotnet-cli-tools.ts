...
const text = (out: string, err: string, code: number) => (code === 0 ? out || err : `Exit ${code}\n${err}\n${out}`);

export function registerDotNetCliTools(server: Server): void {
        ...
        try {
            ...
            switch (name) {
                case 'dotnet_build': {
                    ...
                    if (noRestore) argsArr.push('--no-restore');
                ...
                case 'dotnet_list_projects': {
                    ...
                    if (path && existsSync(path)) {
                        ...
                        if (sln) path = join(path, sln.name);
                    }
                ...
                case 'dotnet_publish': {
                    ...
                    const output = a.output as string | undefined;
                    const argsArr = ['publish', projectPath, '-c', configuration];
                    if (output) argsArr.push('-o', output);
                    const r = await run('dotnet', argsArr);
                    return {
                        content: [{ type: 'text', text: text(r.stdout, r.stderr, r.code) }],
                        isError: r.code !== 0,
                ...
                case 'dotnet_test': {
                    ...
                    if (noBuild) argsArr.push('--no-build');
                    if (filter) argsArr.push('--filter', filter);
                    const r = await run('dotnet', argsArr);
                    return {
                        content: [{ type: 'text', text: text(r.stdout, r.stderr, r.code) }],
                        isError: r.code !== 0,
