...
export function registerSystemInfoTools(server) {
        ...
        try {
            switch (name) {
                case 'sysinfo_get_system': {
                    const categories = a.categories ?? ['os', 'hardware', 'network'];
                    const parts = [];
                    if (categories.includes('os')) {
                        const r = await runPs("Get-CimInstance Win32_OperatingSystem | Select-Object Caption,Version,BuildNumber,OSArchitecture | ConvertTo-Json");
                        parts.push('OS: ' + (r.stdout || r.stderr));
                    }
                    if (categories.includes('hardware')) {
                        const r = await runPs("Get-CimInstance Win32_ComputerSystem | Select-Object Manufacturer,Model,NumberOfLogicalProcessors | ConvertTo-Json");
                        parts.push('Hardware: ' + (r.stdout || r.stderr));
                    }
                    if (categories.includes('memory')) {
                        const r = await runPs("Get-CimInstance Win32_PhysicalMemory | Measure-Object -Property Capacity -Sum | Select-Object @{N='TotalGB';E={[math]::Round($_.Sum/1GB,2)}} | ConvertTo-Json");
                        parts.push('Memory: ' + (r.stdout || r.stderr));
                    }
                    if (categories.includes('disk')) {
                        const r = await runPs("Get-CimInstance Win32_LogicalDisk -Filter \"DriveType=3\" | Select-Object DeviceID,Size,FreeSpace,FileSystem | ConvertTo-Json");
                        parts.push('Disk: ' + (r.stdout || r.stderr));
                    }
                    if (categories.includes('network')) {
                        const r = await runPs("Get-CimInstance Win32_NetworkAdapterConfiguration -Filter 'IPEnabled=True' | Select-Object Description,IPAddress,MACAddress | ConvertTo-Json");
                ...
                case 'sysinfo_get_services': {
                    const svcName = a.name;
                    const status = a.status;
                    let cmd = 'Get-Service';
                    if (svcName)
                        cmd += ` -Name '${escapeSingle(svcName)}'`;
                    if (status && status !== 'All')
                        cmd += ` | Where-Object Status -eq '${status}'`;
                ...
                case 'sysinfo_get_drivers': {
                    ...
                    if (signedOnly)
                        cmd = 'Get-WmiObject Win32_PnPSignedDriver | Where-Object { $_.IsSigned -eq $true } | Select-Object DeviceName,DriverVersion -First 200';
                    cmd += ' | ConvertTo-Json';
                    const r = await runPs(cmd);
                    return { content: [{ type: 'text', text: r.stdout || r.stderr }] };
                }
                ...
                case 'sysinfo_get_software': {
                    ...
                    if (nameFilter)
                        cmd += ` | Where-Object DisplayName -like '*${escapeSingle(nameFilter)}*'`;
                    cmd += ' | ConvertTo-Json';
                    const r = await runPs(cmd);
                    return { content: [{ type: 'text', text: r.stdout || r.stderr }] };
                }
