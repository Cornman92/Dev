            ...
            switch (name) {
                case 'lint_analyze_powershell': {
                    ...
                    const psArgs = [
                        '-NoProfile',
                        '-Command',
                        `Invoke-ScriptAnalyzer -Path '${path.replace(/'/g, "''")}'${recurse ? ' -Recurse' : ''}${fix ? ' -Fix' : ''}${includeDefault ? '' : ' -ExcludeDefaultRules'}${severity?.length ? ` -Severity @(${severity.map((s) => "'" + s + "'").join(',')})` : ''} | ConvertTo-Json -Depth 5`,
                    ];
