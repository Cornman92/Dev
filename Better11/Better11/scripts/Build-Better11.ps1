...
if (-not $MsBuildExe -or -not (Test-Path $MsBuildExe)) {
    $MsBuildExe = (Get-ChildItem -Path "${env:ProgramFiles}\Microsoft Visual Studio" -Recurse -Filter 'MSBuild.exe' -ErrorAction SilentlyContinue | Select-Object -First 1)?.FullName
}
...
if ($Sign -and $Package) {
    ...
    if (-not $certPath -or -not (Test-Path $certPath)) {
        ...
        if ($msixPath) {
            $args = @('sign', '/fd', 'SHA256', '/f', $certPath, '/tr', 'http://timestamp.digicert.com', '/td', 'SHA256')
