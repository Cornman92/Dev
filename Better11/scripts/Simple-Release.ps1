# Simple Better11 Release Build Script
...
try {
    ...
    $appSuccess = (dotnet build src\Better11.App\Better11.App.csproj -c $Configuration); $LASTEXITCODE -eq 0
    
    if ($appSuccess) {
        Write-Host "  ✅ App built successfully" -ForegroundColor Green
    } else {
        Write-Host "  ⚠️  App build failed (expected XAML issue)" -ForegroundColor Yellow
    }
