 ...
 * Used by code-analysis, powershell, system-info, winget, dotnet-cli, nuget MCP servers.
...
export async function isWinGetAvailable(): Promise<boolean> {
    try {
        const r = spawnSync('winget', ['--version'], {
            encoding: 'utf8',
            timeout: 5000,
            windowsHide: true,
        });
    ...
    const server: ToolCaptureServer = {
        setRequestHandler(schema: unknown, handler: ListToolsHandler | CallToolHandler) {
            ...
            if (method === 'tools/list') {
                ...
                // Fallback: many SDKs pass opaque schema; first = list, second = call
                callCount++;
